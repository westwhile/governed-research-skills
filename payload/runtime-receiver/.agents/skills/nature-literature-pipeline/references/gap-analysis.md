...
Search the exact topic string across at least 3 sources, using whatever web
search/extract tooling the current runtime provides (delegated literature
search goes through `nature-academic-search`):
...
Write to a user-approved output location (default proposal: `outputs/literature/<topic>_gap_report.md` — confirm the path with the user before writing) with these sections:
...
- Search: ≤4 parallel web-search calls per decomposition level
- Extract: only edge papers (≤5), not every hit
- No browser: use web-extract style tooling only, skip paywalled pages silently
