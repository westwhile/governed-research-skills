...
Delivery is boundary-gated: send the formatted digest to a messaging target
only when the user explicitly authorizes that run and names a target they have
already configured (see `references/scheduling-and-delivery.md`). Never request
or accept tokens, webhooks, or chat credentials in chat. Without explicit
authorization, present the digest in chat and note that delivery was skipped.
