...
description: >-
  ...
  ID conversion) via public APIs (PubMed, CrossRef, arXiv, OpenAlex) with an
  optional, separately-approved MCP layer (Scopus/ScienceDirect).
  Use for coordinated literature workflows beyond a single lookup, including 文献检索、
...
Report specific retrieval-tool failures and continue with remaining tools; broaden terms when there are no results.

For citation-file export, a non-Windows platform rejection is a terminal failure: stop file export, do not retry it, and do not switch to manual file generation or another writer. At most, return citation text in chat labeled `text only; no file created`; this chat response is not a citation-file export.

On Windows, if the converter fails twice, manual citation-file content may be written only through the converter's public `write_text_exclusive` writer and the same approved output directory. Never use an alternative write path.
...
Read `references/runtime-boundary.md` before any retrieval. This installed profile performs live retrieval only against anonymous public scholarly APIs (OpenAlex, Crossref, NCBI E-utilities, arXiv) and the host's built-in web search, under bounded, rate-limited requests. The bundled MCP server layer is not registered in this environment and must not be installed or registered without separate explicit approval. Credential-gated sources (Elsevier/Scopus/ScienceDirect), Web of Science, CNKI/万方 login paths, and Zotero write access require separate provisioning and approval.

For citation-file export, require an exact user-approved output directory. Pass its resolved absolute path with `--output`; require the directory to exist, and do not infer a default path or create a directory on the user's behalf.

Treat `scripts/format-converter.py` citation-file export as Windows-only. On non-Windows platforms it must fail closed before path expansion, retrieval, conversion, or writes because no native writer has been verified. Treat any item or batch error as a non-zero CLI exit and do not report partial output as a successful batch.
