...
- References without identifiers: extract first line as candidate title, mark for title+author resolution via the no-MCP search path below.

## Resolution Priority

The MCP tools formerly listed here are NOT bundled in this environment; use the no-MCP substitutes (see [MCP tools and shared modules](../static/core/tools.md) part (b)). For each extracted reference:
1. DOI → direct Crossref REST `GET https://api.crossref.org/works/{doi}`
2. PMID → direct NCBI E-utilities `efetch.fcgi?db=pubmed&id={pmid}&retmode=xml`
3. arXiv ID → direct arXiv API `https://export.arxiv.org/api/query?id_list={arxiv_id}`
4. Title + first author → `scripts/academic_search.py` (OpenAlex) or Crossref REST `query.bibliographic=`
