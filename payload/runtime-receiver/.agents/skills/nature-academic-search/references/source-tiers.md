...
> **Adopted-profile note (this environment):** the tier tables below describe the full upstream routing model. In this installed profile the allowed set is narrower: anonymous public APIs (OpenAlex, Crossref REST, NCBI E-utilities, arXiv) plus the host's built-in web search. Semantic Scholar API keys, Google Scholar scraping, Scopus/ScienceDirect, Web of Science, and CNKI/万方 login paths are NOT available here — treat any T2/T3 row naming them as "disclose the coverage gap and substitute per `references/runtime-boundary.md`", never as an instruction to scrape or to seek credentials.
...
| Scopus | API-backed via the optional `mcp-server/` layer (Elsevier API key required); otherwise institution proxy | Credential-gated — inert without provisioned keys |
