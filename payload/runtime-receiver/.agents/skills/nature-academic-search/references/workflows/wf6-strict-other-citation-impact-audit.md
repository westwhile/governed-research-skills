...
2. **Retrieve citing papers with source coverage stated.**
   ...
   - No-key cited-by path for this environment: resolve the target work in
     OpenAlex (e.g. via `scripts/academic_search.py`), then list citing papers
     through the public API filter
     `https://api.openalex.org/works?filter=cites:<openalex_work_id>` (e.g. with
     curl), or fall back to the host agent's built-in web search. Name
     OpenAlex / web search as the checked sources and disclose that
     Scopus/WoS/Semantic Scholar were not covered, per the coverage rules
     above.
