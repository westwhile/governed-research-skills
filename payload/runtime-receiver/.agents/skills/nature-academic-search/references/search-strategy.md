...
3. For biomedical topics: map concepts to MeSH terms via direct NCBI E-utilities on `db=mesh` — `pubmed_lookup_mesh` is NOT bundled in this environment (see [MCP tools and shared modules](../static/core/tools.md) part (b))
