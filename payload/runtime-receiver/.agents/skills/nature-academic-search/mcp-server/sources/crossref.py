...
class CrossRefSource:
    ...
    def __init__(self):
        config = get_config()
        # No invented identity: use the CROSSREF_MAILTO env var or config value
        # only when actually set; otherwise send a bare neutral User-Agent.
        mailto = os.environ.get("CROSSREF_MAILTO") or config.crossref_mailto
        user_agent = "academic-search-skill/1.0"
        if mailto:
