...
# Polite-pool contact; set only via --mailto or OPENALEX_MAILTO / CROSSREF_MAILTO.
# Empty means the mailto parameter is omitted entirely — no invented default.
MAILTO = ""
...
def main():
    ...
    # Resolve the polite-pool contact: --mailto > OPENALEX_MAILTO > CROSSREF_MAILTO.
    # When none is set, MAILTO stays empty and the mailto parameter is omitted.
    global MAILTO
    MAILTO = (args.mailto or os.environ.get("OPENALEX_MAILTO")
              or os.environ.get("CROSSREF_MAILTO") or "")
