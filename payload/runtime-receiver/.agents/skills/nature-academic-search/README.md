...
`nature-academic-search` 是面向 Codex CLI / Kimi Code 工作流的学术检索技能包，通过公共 API（OpenAlex、Crossref、NCBI E-utilities / PubMed、arXiv）提供文献检索、引文核对与引用管理能力；Scopus 和 ScienceDirect 由可选的 MCP 层提供（需单独授权与凭据）。
...
本技能以纯文件形式安装到 skills 根目录下，无安装脚本、无构建步骤：

- Codex：`<用户目录>\.agents\skills\nature-academic-search`
- Kimi Code：`<用户目录>\.kimi-code\skills\nature-academic-search`

正常使用不需要任何 API key 或 MCP 服务器：发现检索走 `scripts/academic_search.py`（OpenAlex，纯标准库），元数据获取走对 Crossref REST / NCBI E-utilities / arXiv API 的匿名直连 HTTP，引用文件导出走 `scripts/format-converter.py`。批量操作前可运行 `python scripts/preflight.py` 检查端点可达性。

`format-converter.py` 的安全引用文件导出为 **Windows-only**：在 **non-Windows** 平台会在路径解析、联网、内容转换和写入前 **fail closed**，因为尚无经原生环境验证的安全 writer。任何条目或批次错误都会产生 **non-zero CLI exit**，不能把部分输出报告为成功批次。

## 可选 MCP 层（本环境未注册）

`mcp-server/` 目录是可选层：本环境**未注册**该 MCP 服务器，未经用户单独明确批准，不得安装、注册或启动；启用前还需安装其自身依赖（见 `mcp-server/requirements.txt`）。
